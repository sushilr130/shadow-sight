...
import { ActivityTypeData, ChartData, EmailDomainData, FileTypeData, ManagerActionData, ProcessedActivity, RiskScoreDistribution, SensitiveDataCount, TimeActivityData, UserActivity, UserRiskData, ViolationCount } from '@/types';
...
export const getEmailDomainDistribution = (data: ProcessedActivity[]): EmailDomainData[] => {
  const domainCounts = new Map<string, number>();
  
  data.forEach(activity => {
    if (activity.emailDomain) {
      domainCounts.set(activity.emailDomain, (domainCounts.get(activity.emailDomain) || 0) + 1);
    }
  });
...
export const getActivityByHour = (data: ProcessedActivity[]): TimeActivityData[] => {
...
export const getManagerActions = (data: ProcessedActivity[]): ManagerActionData[] => {
  const actionCounts = {
    'Escalated': 0,
  ...
  data.forEach(activity => {
    ...
    if (action === 'Escalated') actionCounts['Escalated']++;
    else if (action === 'Employee Counselled') actionCounts['Employee Counselled']++;
    else if (action === 'Known Good Activity') actionCounts['Known Good Activity']++;
...
export const getSensitiveDataCounts = (data: ProcessedActivity[]): SensitiveDataCount[] => {
  return [
...
export const getFileTypeCounts = (data: ProcessedActivity[]): FileTypeData[] => {
  return [
    { fileType: 'Documents', count: data.filter(d => d.documents).length },
    { fileType: 'Presentations', count: data.filter(d => d.presentation).length },
    { fileType: 'Spreadsheets', count: data.filter(d => d.spreadsheets).length },
    { fileType: 'ZIP Files', count: data.filter(d => d.zipFiles).length },
  ];
};

export const getDataClassificationByChannel = (data: ProcessedActivity[], channel: 'email' | 'usb' | 'cloud') => {
  return [
    { 
      name: 'Internal Data', 
...
export const getTopUsersWithDataLeakage = (data: ProcessedActivity[]): UserActivity[] => {
...
export const getMonitoringDataByMonth = (data: ProcessedActivity[]) => {
  ...
  data.forEach(activity => {
    ...
    if (month >= 0 && month < 12) {
      ...
      if (activity.performanceImprovementPlan) result[month].performanceImprovementPlan += 1;
      if (activity.productivityMonitored) result[month].productivityMonitored += 1;
    }
  });
...
export const getEmailEnhancedMonitoringByMonth = (data: ProcessedActivity[]) => {
  const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
  const result = months.map(month => ({
    name: month,
    count: 0
